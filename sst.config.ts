...
export default $config({
  ...
  async run() {
    ...
    // In `sst dev`, run the API worker locally via `wrangler dev --local` so that
// outbound `fetch()` from workerd hits the local Neon HTTP proxy on :4444
// (running the worker on Cloudflare's edge would tunnel fetches through CF,
// returning 1102/1003 for any 127.0.0.1/localhost target). On `sst deploy`,
// use the real `sst.cloudflare.Worker` and a WorkersRoute on pypi.pro.
//
...
if ($dev) {
  ...
  apiUrl = dev.url ?? "http://localhost:8787";
    ...
    // Parse the Neon connection string into Hyperdrive origin fields. Hyperdrive
    // pools origin connections at the Cloudflare edge, removing per-request TLS
    // handshake + auth round-trips that the neon-http driver pays each fetch.
    ...
    const hyperdrive = new sst.cloudflare.Hyperdrive("Database", {
      ...
      // Search results must reflect freshly ingested rows (POST /ingest,
      // /bootstrap). Disable Hyperdrive's SQL response cache to avoid staleness.
      caching: false,
    });
